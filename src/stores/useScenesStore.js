...
export const useScenesStore = create((set, get) => {
  const history = createHistorySlice(set, get, [createDefaultScene()], 'scenes', [
    'currentSceneId',
  ]);
  ...
  return {
    ...
    getCurrentScene: () =>
      get().scenes.find((s) => s.id === get().currentSceneId) || get().scenes[0],
    ...
    pasteObject: () => {
      ...
      if (clipboard.type === 'multi') {
        newObjects = clipboard.objects.map((obj) => {
