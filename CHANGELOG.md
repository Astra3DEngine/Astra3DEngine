...
- **对象操作后消失**：
  ...
  - 修复：使用 `currentScene?.objects` 直接获取对象列表
- **按钮栏位置错误**：
  - 按钮栏在面板左侧而不是最右侧
  - 修复：删除 `flex-direction: row-reverse`
- **激活指示条位置错误**：
  - 激活指示条在按钮左侧而不是右侧
  - 修复：改为 `right: 0`
...
- **版本号元数据文件**：
  ...
  - `PROJECT_FORMAT_VERSION`：项目格式版本号（1.0.0）
  - 辅助函数：`getEngineInfo()`、`getProjectFormatVersion()`、`isProjectFormatCompatible()`
- **场景面板独立化**：
  - 创建 `ScenePanel.jsx` 组件，从 HierarchyPanel 中分离场景管理功能
  - 场景列表显示（带主场景标识星星图标）
  - 场景切换、新建、删除、重命名、设置主场景功能
  - 当前场景信息显示
  - 场景操作按钮（悬停时显示）
- **面板布局改为占比方式**：
  - 场景面板和预制件面板改为使用占比（ratio）而不是固定高度
  - 默认占比：场景面板 15%（范围 10%-40%），预制件面板 20%（范围 10%-35%）
  - 拖拽时动态计算占比变化，适应不同窗口大小
  - 展开时自动填充或使用占比高度
- **样式统一**：
  - 场景面板列表项样式与层级面板保持一致
...
- **场景面板折叠行为异常**：
  - 单独折叠场景面板时显示为 vertical 模式（左侧一列），不符合 VSCode 界面风格
  - 修复：改为只有在所有面板都折叠时才显示为 vertical 模式
- **拖拽方向反了**：
  - 场景面板拖拽方向反了（向下拖拽减小占比）
  - 修复：改为 `delta = clientY - startY`，向下拖拽增大占比
- **prefabsPanelDragRef 重复声明**：
  - 在 App.jsx 中重复声明了 `prefabsPanelDragRef`
  - 修复：删除重复的声明
- **拖拽分隔条消失**：
  - 修改折叠逻辑后拖拽分隔条消失了
  - 修复：修改分隔条显示条件，只在两个面板都展开时显示
